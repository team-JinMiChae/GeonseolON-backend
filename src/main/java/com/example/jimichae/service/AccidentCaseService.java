...
import com.example.jimichae.controller.chatbot.request.SenderType;
import com.example.jimichae.controller.chatbot.response.ChatResponse;
import com.example.jimichae.dto.request.AccidentCaseRequest;
...
public class AccidentCaseService {
    ...
    public AccidentCaseService(AccidentCaseProperties accidentCaseProperties, GithubProperties githubProperties, AccidentCaseRepository accidentCaseRepository, AccidentCaseCacheRepository accidentCaseCacheRepository, EntityManager em) {
    ...
    @Transactional
    public void processContents(AccidentCaseRequest request) {
         List<String> response = parseResponse(request);
        if (!response.isEmpty()) {
            saveData(response, getEmbeddedString(response));
        }
    }
    ...
    public ChatResponse getQuestion(List<ChatRequest> requests, HttpServletRequest request) {
        ...
        if (accidentCaseCacheRepository.existsByKeyword(keyword)){
            try {
                ...
                ragResult = getEmbeddedString(keyword);
    ...
    private List<String> parseResponse(AccidentCaseRequest request) {
        String encodedBusiness = URLEncoder.encode("건설업", StandardCharsets.UTF_8);
        if (request.getKeyword()!= null) {
            request.setKeyword(URLEncoder.encode(request.getKeyword(), StandardCharsets.UTF_8));
        }
        ...
        if (response != null && response.getBody() != null && response.getBody().getItems() != null) {
            return response.getBody().getItems().getItem().stream()
                .map(item -> normalizeText(item.getKeyword()+item.getContents())).toList();
        }
        return List.of();
    }

    private List<Float[]> getEmbeddedString(List<String> originMessages) {
        var client = new EmbeddingsClientBuilder()
                .credential(new AzureKeyCredential(accidentCaseProperties.getEmbeddingKey()))
            .endpoint(accidentCaseProperties.getEmbeddingUrl())
            .buildClient();
       EmbeddingsResult result =  client.embed(originMessages,null,null,null,"text-embedding-3-small",null);
    return  result.getData().stream().map(embedding -> embedding.getEmbeddingList().toArray(new Float[0])).toList();
    }

    private void saveData(List<String> data, List<Float[]> embedding) {
        List<AccidentCase> accidentCases = new ArrayList<>();
        for (int i = 0; i < data.size(); i++) {
            if (accidentCaseRepository.existsAccidentCaseByTheVector(embedding.get(i))) {
                continue;
            }
            accidentCases.add(new AccidentCase(0L, embedding.get(i), data.get(i)));
        }
        accidentCaseRepository.saveAll(accidentCases);
    }

    private List<ChatRequest> getEmbeddedString(String question) {
        final Float[] vector = getEmbeddedString(List.of(question)).getFirst();
        return em.createQuery( "select e.originalText from AccidentCase e order by cosine_distance(e.theVector, :vec) asc limit 5", String.class )
    ...
    private final String GET_KEYWORD_PROMPT = String.join("\n",
        ...
        "3. You must only respond with a single keyword that represents the core subject of the user's question.",
